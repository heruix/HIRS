...
from system_test_core import HIRSPortal, AttestationCAPortal, collectors, \
	...
	run_hirs_provisioner_tpm2, run_hirs_provisioner, parse_xml_with_stripped_namespaces, \
	get_current_timestamp, get_all_nodes_recursively, touch_random_file_and_remove, \
	get_random_pcr_hex_value, is_ubuntu_client, is_tpm2, is_tpm1_2, \
...
if suffix != None:
    print("Configuring with suffix {0}".format(suffix))
    suffix = "-" + suffix
else:
    suffix = ""
...
EK_CA_CERT_LOCATION = "/HIRS/.ci/integration-tests/certs/ek_ca.crt"
	...
	def test_13_tpm2_initial_provision(self):
		...
		logging.info("*****************beginning of initial provisioner run *****************")
 		...
 		if is_tpm2(TPM_VERSION):
 		   logging.info("Provisioning with TPM 2.0...")
 		   provisioner_out = run_hirs_provisioner_tpm2(CLIENT)
 		elif is_tpm1_2(TPM_VERSION):
 		   logging.info("Provisioning with TPM 1.2...")
 		   provisioner_out = run_hirs_provisioner(CLIENT)
 		else:
 			logging.info("Invalid TPM version")
	...
	def test_15_supply_chain_validation_summary_stored_after_second_provisioning(self):
		...
		if is_tpm2(TPM_VERSION):
			...
			if CLIENT_OS == "centos7":
				logging.info("Using TPM 1.2")
				logging.info("Uploading EK CA cert: " + EK_CA_CERT_LOCATION)
